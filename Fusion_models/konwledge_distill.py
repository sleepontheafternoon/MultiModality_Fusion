...
import time
# 这个模型作为教师模型
from utils.model_level import generate_Res_add_model
# 第一个是resnet
...
def F_cross_entropy(preds, targets):
    log_softmax_preds = torch.log(preds)
    loss = torch.mean(torch.sum(-targets * log_softmax_preds, dim=-1))
    return loss


def train_model(model,log_index=0):


    torch.manual_seed(args.seed)

    # 得到已经训练好的教师模型
    teacher_model = generate_Res_add_model(34,n_input_channels=1,n_classes=args.classes)
    teacher_model = torch.nn.DataParallel(teacher_model).cuda()
    checkpoint = torch.load(args.teacher_dir)
    teacher_model.load_state_dict(checkpoint["Resnet"])
    ...
    for epoch in range(args.epoch):
        ...
        for i,data in enumerate(train_loader):
            ...
            t_y = teacher_model(x1,x2)


            loss1 = cross_entropy(y,label)

            loss2 = F_cross_entropy(y,t_y)
            # 学习与标签的差距和教师模型的差距
...
def select_model(model_type=0):
    ...
    args.model_name = "Decision_Resnet"+str(model_type)+"__"
    return model
...
if __name__ == "__main__":
    ...
    train_model(net,log_index=args.log_index)
